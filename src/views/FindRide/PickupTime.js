...
const PickupTime = (props) => {
  ...
  useEffect(() => {
    dispatch(updateTutorialsByTitle("journey_date", date));
  }, [date]);
  ...
  const btnContinueHandler = () => {
    ...
    */
    if (location.state.from === "find") {
      props.history.push("/findride");
    } else if (location.state.from === "tourist") {
      props.history.push("/sundarban");
    }
  };
  ...
  const disablePast = (date) => {
    ...
    return date.getTime() <= now - 80000000;
  };
